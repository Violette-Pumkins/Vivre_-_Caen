            ...
            if($rowCount <= 0) {
                ...
                if($is_active == '1') {
                    if($email_signin == $email && $password_signin == $password) {
                       header("Location: ./index.php?connect=ok");
    ...
    if(isset($_POST["submit"])) {
        ...
        if(!empty($firstname) && !empty($lastname) && !empty($email) && !empty($mobilenumber) && !empty($password)){
            ...
            if($rowCount > 0) {
                ...
                $_first_name = mysqli_real_escape_string($connection, $first_name);
                $_last_name = mysqli_real_escape_string($connection, $last_name);
...
Le mot de passe doit comporter entre 6 et 20 caractères, contenir au moins un caractère spécial, des minuscules, des majuscules et un chiffre.
                ...
                if((preg_match("/^[a-zA-Z ]*$/", $_first_name)) && (preg_match("/^[a-zA-Z ]*$/", $_last_name)) &&
                 ...
                 (preg_match("/^(?=.*\d)(?=.*[@#\-_$%^&+=§!\?])(?=.*[a-z])(?=.*[A-Z])[0-9A-Za-z@#\-_$%^&+=§!\?]{8,20}$/", $_password))){
                    ...
                    // Query
                    $sql = "INSERT INTO users (first_name, last_name, email, mobilenumber, password, token, is_active,
                    date_time) VALUES ('{$first_name}', '{$last_name}', '{$email}', '{$mobilenumber}', '{$password_hash}', 
                    '{$token}', '0', now())";
